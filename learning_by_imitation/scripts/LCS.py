...
#lo siguente es lo que hice en Master ahi se usa para cada nodo aca se deberia usar
...
def compararParamComportamiento(x,y):
    ...
    if diccionario[x]==diccionario[y]:
        ...
        for d in parX: # d tiene el Id de cada SENSOR
            sns = SensorFactory.get(d, parX[d]) # Obtenemos un sensor a partir de las lecturas en parX
...
def cargarEstructuras(idTarea): 
    ...
    if len(diccionario)>0:
        return
    cargado = LoadXML.obtenerGrafo(idTarea)
    ...
    if len (diccionario)==0:   
        diccionario={0:"GoTo",1:"GoTo",2:"Init"}
...
def nuevaDemostracion(topologiaNueva,networkNueva,idTarea):
    ...
    SaveXML.persistirGrafo(idTarea, 'fullDemo', True, diccionario, parComp, topologiaGeneral, networkGeneral)
